= Playing with Dimensions of Coordination within JaCaMo
JaCaMo Team
:toc: right
...
*2.* In the project file (`auction_env.jcm`), create a workspace with an
...
+!start[scheme(Sch)]                        // plan for the goal start defined in the scheme
   <- ?goalState(Sch, auction(AuctionId,S),_,_,_); // retrieve auction Id and service description S
      ...
      focus(ArtId);
      // .broadcast(achieve,focus(Id));         // (*1*) the org will signal the participants about their goals
      start(S)[artifact_id(ArtId)];
      //.at("now + 4 seconds", {+!decide(AuctionId)}); // (*2*) "decide" is now a goal in the scheme
      .
