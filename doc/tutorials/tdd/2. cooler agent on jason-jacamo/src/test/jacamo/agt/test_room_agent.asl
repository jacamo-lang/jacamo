...
+!test_cool_until
    <-
    -+temperature(15); //The default current temperature is 15
    !temperature(10); // We want to reach 10 degrees
    ...
    for ( .range(I,1,10) ) { // Let us check 10x if it is cooling correctly
        ...
        if (C > 10) { // Greater than 10, cooler MUST be on
            !assert_true(cooling);
            -+temperature(C-1); // emulate that the temperature dropped 1 degree
